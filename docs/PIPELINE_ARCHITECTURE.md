...
| Storage | `graph/store.py` | `GraphStore` export seam: Cypher export, JSON |
...
**NetworkX, not a graph database.** No database to install means the
analytics run in-process on any laptop, and the platform (Supabase/Postgres)
is the single source of truth. `CaseGraph` exposes a deliberately narrow
surface (`_add_node`, `_add_edge`, `person_subgraph`, `node`), and
`graph/store.py` is the export seam (Cypher/JSON) — so pointing the graph at
a graph database later would be a contained change if one is ever wanted.
...
```
  ...
  │        │                                              │
  │        ▼                                              │
  │  PostgreSQL (graph source of truth)  ◀──▶  API        │
  │        │                       (FastAPI, mTLS)         │
  │        ▼                    │                          │
  │  Analytics (NetworkX,      Console (React)             │
  │  in-process / batch)       │                           │
  │                                                        │
